...
def regrid_curvature(x, y, n_grid):
    curv = abs_curvature(x, y)
    x_new = piecelin_quantiles(x, curv, np.linspace(0, 1, n_grid))
...
# plotting regridding outputs
fig, ax = plt.subplots()
# ax.set_xlim([-7000, 7000])
ax.plot(x, y)
ax.plot(x_equi, y_equi, color="red", marker="o")
# ax.plot(x_curv, y_curv, color="green")
# ax.plot(x_quant, y_quant)
ax.plot(x_optim, y_optim, color="blue", marker="o")

# Plotting absolute curvature
fig, ax = plt.subplots()
ax.plot(x, y)
ax.plot(x, abs_curvature(x, y))
